...
##A situation in which you woul duse a static graph over a interactive could be if you were publishing your work in a newspaper or magazine.
##the statics would be better because one it it often easier for the general public to understand a static graph over a interactive one if
  
##If you were looking for an answer that pertains more to the data set itself, you could use a static instead if you are trying to show
##change over time, like if you were asked which major had the greatest increase in population over time.
...
server <- function(input, output, session) {
  ...
  df_sub <- reactive({
    ...
    bplot <- reactive({
      boxplot(playerchoice~playerchoice2,data = df_sub, main="Tennis Plater Data",
              xlab="group", ylab="responce")
    })
  })
