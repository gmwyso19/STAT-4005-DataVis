...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(selectInput(inputId = "majorchoice",
...
server <- function(input, output, session) {
  ...
  df_update <- reactive({
    df_stat <- df_major %>% filter(discipline == imput$majorchoice)
  df_statfull <- semi_join(df_long, df_stat, by = "adm_id") %>%
    filter(type == "major1" |
             type == "major2" | 
             type == "major3")
...
output$majorplot <- renderPlot(ggplot(data = df_nostat, aes(x = discipline, y = nstudent)) +
                                 geom_col() +
                                 coord_flip())})
